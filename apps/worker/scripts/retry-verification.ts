...
async function runPublishAttempt(mockStatus: number): Promise<void> {
  const fetchStub = (async () =>
    new Response(JSON.stringify({ error: "stubbed failure" }), {
      status: mockStatus,
    })) as typeof fetch;
...
async function main(): Promise<void> {
  ...
  check(
    "500 retries the configured number of times",
    retryableResult.attemptsMade === attempts,
    `attemptsMade=${retryableResult.attemptsMade}`,
  );
  check(
    "attempted exactly `attempts` times",
    stamps.length === attempts,
    `saw ${stamps.length} attempts`,
  );
  // Exponential from a 5s base: ~5s, ~10s, ~20s, ~40s. Allow slack for scheduler jitter.
  const expected = Array.from(
    { length: (attempts as number) - 1 },
    (_, i) => (backoff.delay / 1000) * 2 ** i,
  );
  const withinTolerance = gaps.every(
    (gap, i) => Math.abs(gap - expected[i]) <= Math.max(2, expected[i] * 0.3),
  );
  check(
    "backoff between attempts matches exponential(5s)",
    withinTolerance,
    `gaps=${gaps}s expected~${expected}s`,
  );

  // Test B: 401 (permanent). Confirm it ends on the first attempt via
  // UnrecoverableError, not the configured 5.
  const permanentDone = new Promise<{ attemptsMade: number; wasUnrecoverable: boolean }>(
    (resolve) => {
      worker.on("failed", (job, error) => {
        if (job?.data.mockStatus === 401) {
          resolve({
            attemptsMade: job.attemptsMade,
            wasUnrecoverable: error instanceof UnrecoverableError,
          });
        }
      });
    },
  ...
  check(
    "401 did not retry (1 attempt, not the configured 5)",
    permanentResult.attemptsMade === 1,
    `attemptsMade=${permanentResult.attemptsMade}`,
  );
  check(
    "no further attempt landed after the failure",
    permanentStamps.length === 1,
    `saw ${permanentStamps.length} attempts`,
  );
