...
| 11  | API platform           | Partial     | Versioned REST at `/api/v1` with hashed API keys, bearer auth failing closed, and in-app key management. Per-key rate limiting. No SDKs, outbound webhooks, or OpenAPI spec.                                                                                  |
...
| 18  | Admin platform         | Partial     | Admin console with platform health, feature flags, and a knowledge review query, all admin-gated in the database. No user management or support tooling.                                                                                                      |
